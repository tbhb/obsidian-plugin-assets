...
const RELEASE_MANAGED_FILES: readonly string[] = [
  'CHANGELOG.md',
...
function checkBuildOutputLeak(files: readonly string[]): void {
  const leaked = files.filter((path) => path.startsWith('dist/'));
  if (leaked.length > 0) {
    fail(
      `Build output committed under \`dist/\`: ${leaked
        .map((name) => `\`${name}\``)
        .join(', ')}. \`dist/\` is gitignored — check your working tree.`,
    );
  }
}
